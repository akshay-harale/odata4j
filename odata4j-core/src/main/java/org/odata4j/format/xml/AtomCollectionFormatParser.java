...
/**
 * The Class AtomCollectionFormatParser is formaat parser for collection type of entities.
 *
...
public class AtomCollectionFormatParser implements FormatParser<OCollection<?>> {
   ...
   * 
   * @param setting
   *            the parseType should be the itemType of the collection
   */
  public AtomCollectionFormatParser(Settings setting) {
    EdmType itemType = ((EdmCollectionType) setting.parseType).getItemType();
    this.settings = new Settings(setting.version, setting.metadata, setting.entitySetName, setting.entityKey,
        setting.isResponse, itemType, setting.parseFunction);
  }
   ...
   *            the metadat.
   * @param isResponse
   *            is this a response. not used for now.
   */
  public AtomCollectionFormatParser(EdmType itemType, EdmDataServices md, boolean isResponse) {
    this.settings = new Settings(null, md, null, null, isResponse, itemType);
  }
  ...
  public OCollection<?> parse(Reader reader) {
    ...
    return AtomCollectionFormatParser.parse(xmlReader, startElementEvent.asStartElement(), this.settings.metadata,
        this.settings.parseType);
  }

  /**
   * Parse the content and returns {@link OCollection} .
   *
   * @param reader the reader
   * @param startElement the start element
   * @param metadata the metadata
   * @param itemType the item type
   * @return the o collection
   */
  public static OCollection parse(XMLEventReader2 reader, StartElement2 startElement, EdmDataServices metadata,
      EdmType itemType) {
    ...
    while (reader.hasNext()) {
      ...
      if (event.isStartElement()
          && event.asStartElement().getName().getNamespaceUri().equals(XmlFormatParser.NS_DATASERVICES)) {
        ...
        if (itemType.isSimple()) {
          OSimpleObject<?> simple = OSimpleObjects.parse((EdmSimpleType) itemType,
              isNull ? null : reader.getElementText());
          rt.add(simple);
        } else if (itemType instanceof EdmComplexType) {
          OComplexObject complex = AtomComplexFormatParser.parse(reader, event.asStartElement(), metadata,
              (EdmComplexType) itemType);
          rt.add(complex);
        } else if (itemType instanceof EdmCollectionType) {
          OCollection col = AtomCollectionFormatParser.parse(reader, startElement, metadata,
              ((EdmCollectionType) itemType).getItemType());
          rt.add(col);
        }
      }
    }

...
public class ODataBatchChangeSetUnit extends ODataBatchUnit {
  ...
  @Override
  public Response execute(HttpHeaders httpHeaders, Providers providers, URI baseUrI) throws Exception {
    if (parts.size() <= 0) {
    ...
    for (ODataBatchSingleUnit unit : parts) {
      ...
      try {
        response = unit.execute(httpHeaders, providers, baseUrI);
      ...
      for (ODataBatchSingleUnit unit : parts) {
        ...
        try {
          ...
          if (response.getStatus() == Status.CREATED.getStatusCode() && response.getEntity() != null && !response.getEntity().equals("")) {
            response = unit.createResponseForBatch(httpHeaders, providers, baseUrI, (String) response.getEntity());
